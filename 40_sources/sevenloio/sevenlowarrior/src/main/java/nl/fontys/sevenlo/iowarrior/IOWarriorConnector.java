 ...
 *
 * This class tries to open a connection to the iowarrior subsystem. On success
 * it creates a handle for each iowarrior found, which can be used to attach to
 * for IO operations. The handles can be used as a parameter (e.g. in the
 * constructor) of classes that provide access to the {@link IOWarrior}s
 * functionality.
 *
...
public final class IOWarriorConnector {
    ...
    static String getSerialNumber( long handle ) {
        return IowKit.getSerialNumber( handle );
    }
    /**
    ...
    /**
     * Singleton helper. See Java concurrency in practice, initialisation
     * idioms. The Holder class is loaded on first invocation. The classloader
     * and the java memory model ensure threadsafety. See Effective Java.
     */
    ...
    /**
     * Get the handle for the i-th warrior. The IOWarriors are sorted in order
     * of product id,serial number. The method will throw an
     * ArrayIndexOutOfBoundsException if no IOWarriors are found or if the
     * method is called with i &gt;= getWarriorCount;
     *
     * @param i the handle index requested
     * @return the handle to an IOWarrior
     * @throws ArrayIndexOutOfBoundsException when no iowarriors are available.
     */
    ...
    /**
     * Get the handle of the first IOWarrior found. The method returns the
     * handle to IOWarrior with the lowest serial number attached to the system
     * before this singleton's constructor was called. The method will throw an
    ...
    /**
     * Helper to keep handles in product id, serial number order. The class
     * definition states that it compares Long. These are compared using the
     * prodict id and serial numbers of the IOWarriors.
     */
    ...
    /**
     * Get the serial number of the warrior. Used in comparison.
     *
    ...
    public String toString() {
        ...
        for ( Long handle : handles ) {
            sb.append( ++j ).append( " - " )
                    .append( "Product = " )
                    .append( Long.toHexString( IowKit.getProductId( handle ) ) )
                    .append( " Serial = " )
                    .append( IowKit.getSerialNumber( handle ) )
                    .append( " Revision = " )
                    .append( Long.toHexString( IowKit.getRevision( handle ) ) )
                    .append( " Handle = " )
                    .append( Long.toHexString( handle ) )
                    .append( "\n" );
        }
        return sb.toString();
    }
